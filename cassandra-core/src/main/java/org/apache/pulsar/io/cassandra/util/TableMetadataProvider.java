...
public class TableMetadataProvider {
    ...
    public static class TableDefinition {
        ...
        public static TableDefinition of(TableId tableId, List<ColumnId> columns,
                                         List<ColumnId> partitionKeyColumns,
                                         List<ColumnId> primaryKeyColumns) {
            return new TableDefinition(tableId, columns, partitionKeyColumns, primaryKeyColumns);
        }
    ...
    public static TableDefinition getTableDefinition(Metadata clusterMetadata, String keyspace, String columnFamily) {
        ...
        KeyspaceMetadata keyspaceMeta = clusterMetadata.getKeyspace(keyspace);
        if (keyspaceMeta == null) {
            throw new IllegalArgumentException("Keyspace '" + keyspace
                    + "' does not exist on this cluster; check the keyspace setting.");
        }
        TableMetadata meta = keyspaceMeta.getTable(columnFamily);
        if (meta == null) {
            throw new IllegalArgumentException("Table '" + columnFamily + "' does not exist in keyspace '"
                    + keyspace + "'; check the columnFamily setting.");
        }
